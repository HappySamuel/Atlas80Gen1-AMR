    ...
    def init_magCB(self, msg):
        if(msg.data!="0" and msg.data!=self.last_msg):
            ...
            if(direction=="in" and int(station)==1):
                self.tmp_command = [0,3,0,4]  # str-skip-str-end
...
#                self.notify_pub.publish("0,1")   # Magnetic Following -- ON
            ...
            # Between Initializing and Ending
                ...
                if(self.command[self.k]==0 and 1<=len(j)<10):
                    self.notify_pub.publish("0,1")   # Magnetic Following -- ON
                    self.moving_forward()
                    self.once = True
                ...
                elif(self.command[self.k]==0 and len(j)>=10):
                    if(self.once==True):
                        self.k += 1
                        self.once = False
                    print "--------------------------------------------------"
            ...
            # Start to Relocalize Vehicle Position until Success
                ...
                self.notify_pub.publish("0,0")   # Magnetic Following -- OFF
                self.stopping()
                if(self.dist_checker(self.position, self.goal_pose, 0.4)==False or self.heading_checker(self.yaw, self.goal_pose.z, 30)==False):
                    self.finish_mag_pub.publish("2")    # On Going
                    print "----------------------------------------"
                    print "|              Relocalize              |"
                    print "----------------------------------------"
                    ...
                    if(self.step_counter == 1):
                        ...
                        self.finish_mag_pub.publish("1")    # Temporary Done
...
if __name__=="__main__":
    rospy.init_node("magnetic_follower_v1_6")
    MagneticFollowerV1_6()
    rospy.spin()
