...
Features :  (1) Enable "Go into" and "Come out" of Bay 1 to 5 by using magnetic following
...
mag_mission = "in-1" or "out-5"

'''

class MagneticFollowerV1_4():
    def __init__(self):
    ...
    def init_magCB(self, msg):
        if(msg.data!="0" and msg.data!=self.last_msg):
            ...
            if(direction=="in" and int(station)==1):
                self.command = [0,3,0,4]  # str-skip-str-end
                self.goal_pose = Point(self.pt_1.x, self.pt_1.y, self.pt_1_heading)
                self.once = True
            elif(direction=="in" and int(station)==2):
                self.command = [0,1,0,2,0,4]  # str-left-str-right-str-end
                self.goal_pose = Point(self.pt_2.x, self.pt_2.y, self.pt_2_heading)
                self.once = True
            elif(direction=="in" and int(station)==3):
                self.command = [0,1,0,3,0,2,0,4]  # str-left-str-skip-str-right-str-end
                self.goal_pose = Point(self.pt_3.x, self.pt_3.y, self.pt_3_heading)
                self.once = True
            elif(direction=="in" and int(station)==4):
                self.command = [0,1,0,3,0,3,0,2,0,4]  # str-left-str-skip-str-skip-str-skip-str-end
                self.goal_pose = Point(self.pt_4.x, self.pt_4.y, self.pt_4_heading)
                self.once = True
            elif(direction=="in" and int(station)==5):
                self.command = [0,1,0,3,0,3,0,3,0,2,0,4]  # str-left-str-skip-str-skip-str-skip-str-skip-str-end
                self.goal_pose = Point(self.pt_5.x, self.pt_5.y, self.pt_5_heading)
                self.once = True
            # Coming out from Bay 1-5
            elif(direction=="out" and int(station)==1):
                self.command = [0,3,0,3,0,4]  # str-skip-str-skip-str-end
                self.goal_pose = Point(self.out_pt.x, self.out_pt.y, self.out_pt_heading)
                self.once = True
            elif(direction=="out" and int(station)==2):
                self.command = [0,3,0,1,0,2,0,4]  # str-skip-str-left-str-right-str-end
                self.goal_pose = Point(self.out_pt.x, self.out_pt.y, self.out_pt_heading)
                self.once = True
            elif(direction=="out" and int(station)==3):
...
if __name__=="__main__":
    rospy.init_node("magnetic_follower_v1_4")
    MagneticFollowerV1_4()
    rospy.spin()
