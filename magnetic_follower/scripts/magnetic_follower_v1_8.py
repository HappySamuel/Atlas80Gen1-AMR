...
Features :  (1) Enable "Go into" and "Come out" of Bay 4 to 5 with magnetic following
            ...
            (10)Station 1 is treated differently
...
class MagneticFollowerV1_8():
    def __init__(self):
        ...
        self.arrive_pub = rospy.Publisher(rospy.get_param("~arrive_topic"), String, queue_size=1)
    ...
    def init_magCB(self, msg):
        if(msg.data!="0" and msg.data!=self.last_msg and msg.data != "cancel"):
            self.k = 0
            print "--------------------------------"
            ...
            if(len(mag_mission)==3):
                ...
                gate = mag_mission[2]
                self.special_station = int(mag_mission[1])
            # Going into the Bay 1-5
            if(self.direction=="in" and int(station)==1):
                self.tmp_command = [0,3,0,3,0,4]  # [0,3,0,4] str-skip-str-skip-str-end
            ...
            # Between Initializing and Ending
                ...
                # Shutdown magnetic following mode and revert back to lidar based waypt_navi
                    ...
                    if(self.abort_iters >= 60):
                        ...
                        print "------------------------------------------------------------------"
                    self.abort_iters += 1
                # Sending "arrived" to navigation_decider
                if(self.direction=="in" and len(self.command)>3 and len(self.command)-4<=self.k<=len(self.command)-1 and self.special_station==1):
                    self.arrive_pub.publish("1")     # "1" <--> "arrived"
            # Start to Relocalize Vehicle Position until Success
            elif(not self.mag_on and self.k==len(self.command)-1 and self.end):
                self.notify_pub.publish("0,0")   # Magnetic Following -- OFF
                self.stopping()
                if(not self.dist_checker(self.position, self.goal_pose, 0.8) or not self.heading_checker(self.yaw, self.goal_pose.z, 30)):
                    self.finish_mag_pub.publish("2")    # On Going
                    print "----------------------------------------"
                    print "|              Relocalize              |"
                    print "----------------------------------------"
...
if __name__=="__main__":
    rospy.init_node("magnetic_follower_v1_8")
    MagneticFollowerV1_8()
    rospy.spin()
