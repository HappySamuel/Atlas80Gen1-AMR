...
void OdoIMU::encodersCallBack(nav_msgs::Odometry encMsg)
{
    ...
    if( fabs(encMsg.twist.twist.linear.x) < 0.005 and fabs(encMsg.twist.twist.angular.z) < 0.01) //the car is not moving
    ...
    if(encMsg.twist.twist.linear.x >= 0.0)
        d_dist = sqrt(pow((now_position_.x-last_position_.x),2) + pow((now_position_.y-last_position_.y),2) + pow((now_position_.z-last_position_.z),2));
    else
        d_dist = -sqrt(pow((now_position_.x-last_position_.x),2) + pow((now_position_.y-last_position_.y),2) + pow((now_position_.z-last_position_.z),2));
...
{
    ros::init(argc, argv, "psa_odom_Imu");
    OdoIMU odomimu;
    ros::spin();
    return 0;
}
