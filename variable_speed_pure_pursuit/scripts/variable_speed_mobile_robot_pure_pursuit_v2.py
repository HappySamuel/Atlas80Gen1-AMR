...
from geometry_msgs.msg import PolygonStamped, Twist, Vector3, PoseWithCovarianceStamped
from visualization_msgs.msg import Marker
from nav_msgs.msg import Odometry


class VariableSpeedMobileRobotPurePursuitV2(object):
    def __init__(self):
	...
	self.lookahead_dist_pub = rospy.Publisher("/pure_pursuit/lookahead_dist", Marker, queue_size=1)
        ...
        self.amcl_sub = rospy.Subscriber(rospy.get_param("~amcl_topic"), PoseWithCovarianceStamped, self.odomCB, queue_size=1)
	...
	if self.lookahead_pt_pub.get_num_connections() > 0 and isinstance(self.lookahead_point, np.ndarray) and isinstance(current_pose, np.ndarray):
            self.lookahead_dist_pub.publish(utils.make_line_marker(
            self.lookahead_point, current_pose, 0.3, [1.0,1.0,0.0], "/map", "/pure_pursuit", 0, 3))
...
if __name__=="__main__":
    rospy.init_node("variable_speed_mobile_robot_pure_pursuit_v2")
    VariableSpeedMobileRobotPurePursuitV2()
    rospy.spin()
