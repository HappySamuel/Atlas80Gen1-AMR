...
#------------------ SAMUEL - added - Decide  Whether to Publish pose_covariance for Robot Localization USE -------------------
        self.PUBLISH_POSE_COVARIANCE = bool(rospy.get_param("~publish_pose_covariance", "1"))
        self.TF_MODE           = int(rospy.get_param("~tf_mode", "1"))
#-----------------------------------------------------------------------------------------------------------------------------
...
#-------------------- SAMUEL - added - Publish PoseWithCovarianceStamped for Robot Localization USE ---------------------
        if self.PUBLISH_POSE_COVARIANCE:
            self.pose_covariance_pub = rospy.Publisher("/pf/pose_covariance", PoseWithCovarianceStamped, queue_size = 1)
#------------------------------------------------------------------------------------------------------------------------
...
#-------------------------- SAMUEL - added - Publish PoseWithCovariance Msg ---------------------------------
        if self.PUBLISH_POSE_COVARIANCE:
            ...
            pose_covariance.pose.pose.orientation = Utils.angle_to_quaternion(pose[2])
            # Copy in the Covariance Matrix, Converting from 3-D to 6-D
            temp_covariance = self.covariance_generator(self.expected_square_pose(), self.square_expected_pose())
            for i in range(0,2):
                for j in range(0,2):
                    pose_covariance.pose.covariance[6*i+j] = temp_covariance[i][j]
        ...
        # Publishing "map -> base_link" TF Transform
            ...
            self.pub_tf.sendTransform((pose[0],pose[1],0),tf.transformations.quaternion_from_euler(0, 0, pose[2]), stamp , "base_link", "map")
        # Publishing "map -> odom" TF Transform
        elif self.TF_MODE == 3:
            try:
                # "odom -> base" transform
                (odom_base_trans, odom_base_rot) = self.tfTL.lookupTransform("odom", "base_link", rospy.Time(0))
                odom_base_trans_mat = tf.transformations.translation_matrix(odom_base_trans)
                odom_base_rot_mat = tf.transformations.quaternion_matrix(odom_base_rot)
                odom_base_mat = np.dot(odom_base_trans_mat, odom_base_rot_mat)
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                return
            # "map -> base" transform
            map_base_trans = [pose[0], pose[1], 0]
            map_base_rot = tf.transformations.quaternion_from_euler(0,0,pose[2])
            map_base_trans_mat = tf.transformations.translation_matrix(map_base_trans)
            map_base_rot_mat = tf.transformations.quaternion_matrix(map_base_rot)
            map_base_mat = np.dot(map_base_trans_mat, map_base_rot_mat)
            # "base -> map" transform
            base_map_mat = tf.transformations.inverse_matrix(map_base_mat)
            base_map_trans = tf.transformations.translation_from_matrix(base_map_mat)
            base_map_rot = tf.transformations.quaternion_from_matrix(base_map_mat)
            # "odom -> map" transform
            odom_map_mat = np.dot(odom_base_mat, base_map_mat)
            # "map -> odom" transform
            map_odom_mat = tf.transformations.inverse_matrix(odom_map_mat)
            map_odom_trans = tf.transformations.translation_from_matrix(map_odom_mat)
            map_odom_rot = tf.transformations.quaternion_from_matrix(map_odom_mat)
            self.pub_tf.sendTransform(map_odom_trans, map_odom_rot, stamp, "odom", "map")
        else:
...
#------------------------------ SAMUEL - added - Compute E(X^2) and E(X)^2, Then Compute Covariance ---------------------------------
    # Compute E(X^2) <----> E_X2
    def expected_square_pose(self):
        return np.dot(np.square(self.particles).transpose(), self.weights)

    # Compute E(X)^2 <----> EX_2
    def square_expected_pose(self):
        return np.square(self.expected_pose())

    # Compute Covariance <----> Covariance = E_X2 - EX_2
    def covariance_generator(self, E_X2, EX_2):
        covariance = np.subtract(E_X2, EX_2)
	# Create 3x3 Covariance Matrix
        nxn_covariance = np.zeros((3,3))
        for i in range(0,3):
            for j in range(0,3):
                if i == j:
                    nxn_covariance[i][j] = covariance[i]
        return nxn_covariance
#----------------------------------------------------------------------------------------------------------------------------------
...
if __name__=="__main__":
    rospy.init_node("tf_particle_filter")
