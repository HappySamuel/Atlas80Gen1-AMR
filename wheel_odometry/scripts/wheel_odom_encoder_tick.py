...
class WheelOdomEncoderTick():
    def __init__(self):
        ...
        # Subscribers
        self.encoder_sub = rospy.Subscriber(rospy.get_param("~encoder_topic"), String, self.encoderCB, queue_size=1)

        # Publisher
        self.wheel_odom_pub = rospy.Publisher(rospy.get_param("~wheel_odom_topic"), Odometry, queue_size=1)
    ...
    def publish_odom(self, x_pos, y_pos, yaw_angle, current_speed, current_yaw_rate):
        wheel_odom = Odometry()
        wheel_odom.header.frame_id = "map"
        wheel_odom.header.stamp = rospy.Time.now()
        wheel_odom.child_frame_id = "base_link"
        wheel_odom.pose.pose.position = Point(x_pos, y_pos, 0.0)
        wheel_odom.pose.pose.orientation = Quaternion(0.0, 0.0, np.sin(yaw_angle/2.0), np.cos(yaw_angle/2.0))
        wheel_odom.pose.covariance[0] = 0.2     # x
        wheel_odom.pose.covariance[7] = 0.2     # y
        wheel_odom.pose.covariance[35] = 0.4    # yaw
        wheel_odom.twist.twist.linear = Vector3(current_speed, 0, 0)
        wheel_odom.twist.twist.angular = Vector3(0, 0, current_yaw_rate)
