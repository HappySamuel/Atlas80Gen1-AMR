...
"""
Changes :  (1) Compatible with temporary_magnetic_follower.py (Bay 4-5 + Exit)
           (2) mag_goal command changed
           (3) Able to do only exit with going to Bay 1-3
           (4) Routes related to "Psa_C" are removed
"""
...
class PsaNavigationDeciderV1_16():
    def __init__(self):
    ...
    def mission_done_checker(self):
        if(self.route != []):
            ...
            if(self.dist_checker(self.c_pos, self.end_pt, self.waypt_tolerance) and self.counter == 1 and self.last_end_pt != self.end_pt):
                self.counter = 2
                del self.route[0]
            ...
            elif(self.dist_checker(self.c_pos, self.end_pt, self.waypt_tolerance) and self.counter == 2 and self.last_end_pt != self.end_pt):
                self.counter = 3
                del self.route[0]
                self.delivery_status = "arrived"
                self.location = self.route[0]
            ...
            elif(self.dist_checker(self.c_pos, self.end_pt, self.waypt_tolerance) and self.counter == 3 and self.last_end_pt != self.end_pt):
...
if __name__=="__main__":
    rospy.init_node("psa_navigation_decider_v1_16")
    PsaNavigationDeciderV1_16()
    rospy.spin()
