...
from geometry_msgs.msg import PolygonStamped, Twist, Point, Quaternion, Vector3
from nav_msgs.msg import Odometry

# "status" : "started / arrived / departed / suspended / resumed / cancelled / completed"
# "lifter" : "up / down"

class NavigationDeciderV5():
    def __init__(self):
        ...
        self.heading_drive_topic = rospy.get_param("~heading_drive_topic")
        ...
        self.check_point = Point(float(rospy.get_param("~check_point_x")), float(rospy.get_param("~check_point_y")), 0)
        ...
        self.path_31 = self.path_directory + "/shado_completed.traj"
        ...
        self.heading_drive_pub = rospy.Publisher(self.heading_drive_topic, Twist, queue_size=1)
    ...
    def missionCB(self, msg):
        if(msg.data != "" and self.battery_safe == True and (self.delivery_status == "completed" or self.delivery_status == "")):
            route_action = []
            self.route = []
            self.action = []
...
#            print "start_check :  " + str(self.start_check)
            ...
            # Check Start New Mission heading
            self.mission_start_checker()
            if(self.heading_check == False):  # Skip below if heading is not correct when mission started
                continue
    ...
    # Check the heading after receiving a new mission
    def mission_start_checker(self):
        if(self.delivery_status == "started" and self.dist_checker(self.c_pose, self.check_point) and self.first_time == True):
    ...
    def tuning_heading(self, rotspeed):
        ...
        self.drive_msg.linear = Vector3(0, 0, 0)
        self.drive_msg.angular = Vector3(rotspeed)
        self.drive_pub.publish(drive_msg)


if __name__=="__main__":
    rospy.init_node("navigation_decider_v5")
    NavigationDeciderV5()
    rospy.spin()
