...
"""
Changes :  (1) Self-charging procedure
           (2) Bug on the message type for voltage_percent
           (3) yaw angle [rad] -> [deg]
"""
...
class PsaNavigationDeciderV1_7a():
    def __init__(self):
        ...
        self.path_7 = self.path_directory + "/psa_A_1.traj"
        self.path_8 = self.path_directory + "/psa_A_2.traj"
        self.path_9 = self.path_directory + "/psa_A_3.traj"
        self.path_10 = self.path_directory + "/psa_B_1.traj"
        self.path_11 = self.path_directory + "/psa_B_2.traj"
        self.path_12 = self.path_directory + "/psa_B_3.traj"
        self.path_13 = self.path_directory + "/psa_C_1.traj"
        self.path_14 = self.path_directory + "/psa_C_2.traj"
        self.path_15 = self.path_directory + "/psa_C_3.traj"
        self.path_16 = self.path_directory + "/psa_1_A.traj"
        self.path_17 = self.path_directory + "/psa_1_B.traj"
        self.path_18 = self.path_directory + "/psa_1_C.traj"
        self.path_19 = self.path_directory + "/psa_2_A.traj"
        self.path_20 = self.path_directory + "/psa_2_B.traj"
        self.path_21 = self.path_directory + "/psa_2_C.traj"
        self.path_22 = self.path_directory + "/psa_3_A.traj"
        self.path_23 = self.path_directory + "/psa_3_B.traj"
        self.path_24 = self.path_directory + "/psa_3_C.traj"
        self.path_25 = self.path_directory + "/psa_A_home.traj"
        self.path_26 = self.path_directory + "/psa_B_home.traj"
        self.path_27 = self.path_directory + "/psa_C_home.traj"
        self.path_28 = self.path_directory + "/psa_1_home.traj"
        self.path_29 = self.path_directory + "/psa_2_home.traj"
        self.path_30 = self.path_directory + "/psa_3_home.traj"
        self.path_31 = self.path_directory + "/psa_completed.traj"
        self.sound_file = "/home/atlas80-b/Desktop/countdown_10_seconds.ogg"

        # Internal Use Variables - Do not modify without consultation
        self.pub_rate = rospy.Rate(2)    # 0.5 [Hz] <---> 2 [sec]
        ...
        self.time_counter = 0
        self.timer = self.wait_time * 2    # [count = sec * Hz]
    ...
    def mission_start_checker(self):
        if(self.delivery_status == "started" and self.dist_checker(self.c_pose, self.check_point)):
    ...
    def mission_done_checker(self):
        if(self.route != []):
            ...
            if(self.dist_checker(self.c_pose, self.end_pt) and self.counter == 1 and (self.last_end_pt[0] != self.end_pt[0] or self.last_end_pt[1] != self.end_pt[1])):
                self.counter = 2
                del self.route[0]
            ...
            elif(self.dist_checker(self.c_pose, self.end_pt) and self.counter == 2 and (self.last_end_pt[0] != self.end_pt[0] or self.last_end_pt[1] != self.end_pt[1])):
                self.counter = 3
                del self.route[0]
                self.delivery_status = "arrived"
                self.location = self.route[0]
            ...
            elif(self.dist_checker(self.c_pose, self.end_pt) and self.counter == 3 and (self.last_end_pt[0] != self.end_pt[0] or self.last_end_pt[1] != self.end_pt[1])):
                    ...
                    # Initiating - (2) Stop & Go
                        ...
                        if(self.time_counter < self.timer):
                            if(self.time_counter == 0):
                                self.toggle_suspend_resume()
                            if(remaining_time == 22 and self.play_counter == 1):
    ...
    def obstacle_detector(self):
        ...
        elif(int(self.lifter) == 0 and self.obstacle_mode == 1 and self.dist_checker(self.c_pose, self.start_pt) == False):  # Without Table On Top
...
if __name__=="__main__":
    rospy.init_node("psa_navigation_decider_v1_7a")
    PsaNavigationDeciderV1_7a()
    rospy.spin()
